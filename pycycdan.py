...
#            self.data = d[:ntot,:,:,:].reshape((nsub,tscrunch,d.shape[1],d.shape[2],d.shape[3])).mean(1)
	...
	self.loadFile = loadFile
        ...
        if loadFile:
            x = self.pp_ref.argmax()
            y = pp.argmax()
            #diff=x-y+1
            diff=x-y
            self.pp_ref = np.roll(self.pp_ref,-diff)
            x = self.pp_ref.argmax()
            self.pp_ref = np.roll(self.pp_ref,-x)

            ps = self.data[isub,ipol]
            ps = np.roll(ps,-x,axis=1)
            cs = ps2cs(ps)
            cs = normalize_cs(cs,bw=self.bw,ref_freq=self.ref_freq)
            cs = cyclic_padding(cs,self.bw,self.ref_freq)
            hf = np.ones((self.nchan,),dtype='complex')
    ...
    def plotCurrentSolution(self):
        ...
        im = ax1b.imshow(np.angle(self.cs[:,:mlag]),cmap=plt.cm.hsv,aspect='auto',interpolation='nearest',extent=csextent)
        ...
        im = ax2b.imshow(np.angle(cs_model[:,:mlag]),cmap=plt.cm.hsv,aspect='auto',interpolation='nearest',extent=csextent)
...
def plotSimulation(self,mlag=100):
    if CS.ht0 is None:
